...
bool BlobReader::read_str(std::string *dest, uint32_t len, std::string_view desc) {
...
bool BlobReader::decompress_zlib(std::vector<uint8_t> *dest, std::string_view desc) {
  z_stream z{};
  int status = 0;
  int ret = 0;
  int bytes_read = 0;
  ...
  while (true) {
    ...
    if (DEBUG) {
      std::cout << "inflate: status: " << status << ", read: "
                << bytes_read << ", written: " << bytes_written << "\n";
    }
